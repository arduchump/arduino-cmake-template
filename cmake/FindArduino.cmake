...
function(split_preference_key ASTRING OUT_KEY)
...
function(split_preference_value ASTRING OUT_VALUE)
...
function(split_preference ASTRING OUT_KEY OUT_VALUE)
    split_preference_key("${ASTRING}" GOT_KEY)
    split_preference_value("${ASTRING}" GOT_VALUE)

    set(${OUT_KEY} "${GOT_KEY}" PARENT_SCOPE)
    set(${OUT_VALUE} "${GOT_VALUE}" PARENT_SCOPE)
endfunction()

function(get_preference ALIST KEY OUT_VALUE)
    set(${OUT_VALUE} "" PARENT_SCOPE)
    foreach(PREFERENCE ${ALIST})
        split_preference("${PREFERENCE}" GOT_KEY GOT_VALUE)
...
function(arduino_get_preferences OUT_PREFERENCES)
    execute_process(
    ...
    get_preference("${ARDUINO_PREFERENCES}" "runtime.ide.path" ARDUINO_IDE_PATH)
    get_preference("${ARDUINO_PREFERENCES}" "runtime.platform.path" ARDUINO_PLATFORM_PATH)
...
arduino_get_preferences(ARDUINO_PREFERENCES)
